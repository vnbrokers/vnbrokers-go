...
func main() {
	...
	request := nativedto.SubscribeDerivativeTickerMatchesRequest{Symbols: env.List("TCBS_SYMBOLS", "VN30F1M")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeDerivativeTickerMatches(ctx, request)
	if err != nil {
		panic(err)
	}
