...
func main() {
	...
	request := nativedto.CancelDerivativeConditionalOrderRequest{CancelDerivativeConditionalOrderBody: nativedto.CancelDerivativeConditionalOrderBody{AccountID: mustEnv("TCBS_ACCOUNT_ID"), OrderNo: mustEnv("TCBS_ORDER_NO"), SubAccountID: mustEnv("TCBS_SUB_ACCOUNT_ID")}}
	response, err := broker.Native().Trading().CancelDerivativeConditionalOrder(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
