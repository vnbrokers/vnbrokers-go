...
func main() {
	...
	request := nativedto.GetStockMatchingDetailsRequest{AccountNo: env.RequiredString("TCBS_ACCOUNT_NO")}
	response, err := broker.Native().Trading().GetStockMatchingDetails(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
