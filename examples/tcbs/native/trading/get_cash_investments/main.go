...
func main() {
	...
	request := nativedto.GetCashInvestmentsRequest{AccountNo: env.RequiredString("TCBS_ACCOUNT_NO")}
	response, err := broker.Native().Trading().GetCashInvestments(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
