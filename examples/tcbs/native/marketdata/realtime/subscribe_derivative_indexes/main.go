...
func main() {
	...
	request := nativedto.SubscribeDerivativeIndexesRequest{Indexes: env.List("TCBS_INDEXES", "VN30")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeDerivativeIndexes(ctx, request)
	if err != nil {
		panic(err)
	}
