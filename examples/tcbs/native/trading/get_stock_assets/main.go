...
func main() {
	...
	request := nativedto.GetStockAssetsRequest{AccountNo: env.RequiredString("TCBS_ACCOUNT_NO")}
	response, err := broker.Native().Trading().GetStockAssets(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
