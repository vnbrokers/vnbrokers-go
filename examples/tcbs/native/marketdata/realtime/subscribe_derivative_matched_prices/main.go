...
func main() {
	...
	request := nativedto.SubscribeDerivativeMatchedPricesRequest{Symbols: env.List("TCBS_SYMBOLS", "VN30F1M")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeDerivativeMatchedPrices(ctx, request)
	if err != nil {
		panic(err)
	}
