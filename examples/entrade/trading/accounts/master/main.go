...
func main() {
	response, err := exampleutil.Broker().Native().Trading().GetInvestorAccount(
		context.Background(),
		nativedto.GetInvestorAccountRequest{InvestorID: exampleutil.MustEnv("ENTRADE_INVESTOR_ID")},
	)
	if err != nil {
		panic(err)
	}
