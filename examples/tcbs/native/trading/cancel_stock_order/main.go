...
func main() {
	...
	request := nativedto.CancelStockOrderRequest{AccountNo: mustEnv("TCBS_ACCOUNT_NO"), CancelStockOrderBody: nativedto.CancelStockOrderBody{OrdersList: []nativedto.StockOrderID{{OrderID: mustEnv("TCBS_ORDER_ID")}}}}
	response, err := broker.Native().Trading().CancelStockOrder(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
