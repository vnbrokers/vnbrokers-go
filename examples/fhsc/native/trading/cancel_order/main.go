...
func main() {
	...
	request := nativedto.CancelOrderOperationRequest{
		SubAccountID: env.RequiredString("FHSC_SUB_ACCOUNT_ID"),
		OrderID:      env.RequiredString("FHSC_ORDER_ID"),
		Body: nativedto.CancelOrderRequest{
			SubAccount: env.RequiredString("FHSC_SUB_ACCOUNT_EXT_ORDER"),
		},
	}
	response, err := broker.Native().Trading().CancelOrder(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
