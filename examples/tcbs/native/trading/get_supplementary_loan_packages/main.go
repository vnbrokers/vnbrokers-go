...
func main() {
	...
	request := nativedto.GetSupplementaryLoanPackagesRequest{AccountNo: env.RequiredString("TCBS_ACCOUNT_NO")}
	response, err := broker.Native().Trading().GetSupplementaryLoanPackages(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
