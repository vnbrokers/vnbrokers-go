...
func main() {
	...
	request := nativedto.GetTradingEconomicsDataRequest{
		Country:  env.String("FHSC_COUNTRY", "vietnam"),
		Category: env.String("FHSC_CATEGORY", ""),
		Year:     env.Int("FHSC_YEAR", 0),
	}
	response, err := broker.Native().MarketData().GetTradingEconomicsData(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
