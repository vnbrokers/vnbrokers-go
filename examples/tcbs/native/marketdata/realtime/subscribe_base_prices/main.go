...
func main() {
	...
	request := nativedto.SubscribeBasePricesRequest{Symbols: env.List("TCBS_SYMBOLS", "TCX")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeBasePrices(ctx, request)
	if err != nil {
		panic(err)
	}
