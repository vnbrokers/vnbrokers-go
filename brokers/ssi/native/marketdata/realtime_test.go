...
func TestMarketDataRealtimeRequiresDataAccessToken(t *testing.T) {
	broker := ssi.NewBroker(ssi.Config{TradingToken: "trading-token"})
...
func TestMarketDataRealtimeTypedChannels(t *testing.T) {
	tests := []struct {
		name      string
		channel   string
		subscribe func(*ssi.Broker) error
	}{
		{
			name:    "trading status",
			channel: "F:SSI-PAN",
			subscribe: func(broker *ssi.Broker) error {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			broker := ssi.NewBroker(ssi.Config{
				...
				SignalRFactory: func(string, []string) ssi.SignalRClient { return fake },
			})
...
func TestMarketDataRealtimePublishesTypedEvents(t *testing.T) {
		newBroker := func() (*ssi.Broker, *fakeSignalRClient) {
		fake := newFakeSignalRClient()
		broker := ssi.NewBroker(ssi.Config{
			DataToken:    "data-token",
			SignalRFactory: func(string, []string) ssi.SignalRClient { return fake },
	...
	t.Run("trading status", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "broadcast", nestedSSIArg(t, "F", map[string]any{
	...
	t.Run("quote", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "update", nestedSSIArg(t, "X-QUOTE", map[string]any{
	...
	t.Run("trade", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "broadcast", nestedSSIArg(t, "X-TRADE", map[string]any{
	...
	t.Run("snapshot", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "update", nestedSSIArg(t, "X", map[string]any{
	...
	t.Run("foreign room", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "broadcast", nestedSSIArg(t, "R", map[string]any{
	...
	t.Run("market index", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "update", nestedSSIArg(t, "MI", map[string]any{
	...
	t.Run("ohlcv", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "broadcast", nestedSSIArg(t, "B", map[string]any{
			"RType": "B", "Symbol": "SSI", "Time": "14:28:33", "Volume": 5000,
		}))
		event := receiveEvent(t, subscription.Events())
		if event.RType != "B" || event.Symbol != "SSI" || event.Time != "14:28:33" || event.Volume != 5000 {
			t.Fatalf("event = %+v", event)
		}
	})
	...
	t.Run("odd lot", func(t *testing.T) {
		...
		fake.emit("FcMarketDataV2Hub", "update", nestedSSIArg(t, "OL", map[string]any{
...
func TestMarketDataRealtimeReportsTypedDecodeErrors(t *testing.T) {
	fake := newFakeSignalRClient()
	broker := ssi.NewBroker(ssi.Config{
		DataToken: "data-token",
		SignalRFactory: func(string, []string) ssi.SignalRClient { return fake },
	...
	fake.emit("FcMarketDataV2Hub", "broadcast", nestedSSIArg(t, "X-QUOTE", map[string]any{
...
func TestMarketDataRealtimePublishesRawChannel(t *testing.T) {
	fake := newFakeSignalRClient()
	broker := ssi.NewBroker(ssi.Config{
		DataToken: "data-token",
		SignalRFactory: func(string, []string) ssi.SignalRClient { return fake },
