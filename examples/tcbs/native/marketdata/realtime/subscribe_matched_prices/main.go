...
func main() {
	...
	request := nativedto.SubscribeMatchedPricesRequest{Symbols: env.List("TCBS_SYMBOLS", "VN30F1M")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeMatchedPrices(ctx, request)
	if err != nil {
		panic(err)
	}
