...
func main() {
	...
	sub, err := broker.Native().MarketData().Realtime().SubscribeQuotes(context.Background(), nativedto.SubscribeSymbolsRequest{Symbols: []string{"ACB", "41I1G6000"}, BoardID: "G1"})
	if err != nil {
		panic(err)
	}
	defer sub.Close()
