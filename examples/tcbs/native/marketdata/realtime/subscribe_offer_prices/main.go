...
func main() {
	...
	request := nativedto.SubscribeOfferPricesRequest{Symbols: env.List("TCBS_SYMBOLS", "VN30F1M")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeOfferPrices(ctx, request)
	if err != nil {
		panic(err)
	}
