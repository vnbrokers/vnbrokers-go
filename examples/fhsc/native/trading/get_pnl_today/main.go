...
func main() {
	...
	request := nativedto.GetPnLTodayRequest{
		UserID:       env.RequiredInt("FHSC_USER_ID"),
		SubAccountID: env.String("FHSC_SUB_ACCOUNT_ID", ""),
	}
	response, err := broker.Native().Trading().GetPnLToday(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
