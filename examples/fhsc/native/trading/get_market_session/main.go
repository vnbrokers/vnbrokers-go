...
func main() {
	...
	request := nativedto.GetMarketSessionRequest{
		Exchange: env.String("FHSC_EXCHANGE", "HOSE"),
	}
	response, err := broker.Native().Trading().GetMarketSession(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
