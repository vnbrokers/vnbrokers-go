...
func main() {
	...
	request := nativedto.SubscribeDerivativeOfferPricesRequest{Symbols: env.List("TCBS_SYMBOLS", "VN30F1M")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeDerivativeOfferPrices(ctx, request)
	if err != nil {
		panic(err)
	}
