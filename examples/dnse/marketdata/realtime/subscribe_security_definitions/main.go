...
func main() {
	...
	sub, err := broker.Native().MarketData().Realtime().SubscribeSecurityDefinitions(context.Background(), nativedto.SubscribeSymbolsRequest{Symbols: []string{"ACB"}, BoardID: "G1"})
	if err != nil {
		panic(err)
	}
	defer sub.Close()
