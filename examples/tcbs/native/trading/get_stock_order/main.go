...
func main() {
	...
	request := nativedto.GetStockOrderRequest{AccountNo: mustEnv("TCBS_ACCOUNT_NO"), OrderID: mustEnv("TCBS_ORDER_ID")}
	response, err := broker.Native().Trading().GetStockOrder(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
