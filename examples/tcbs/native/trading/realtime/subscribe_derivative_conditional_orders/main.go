...
func main() {
	...
	request := nativedto.SubscribeDerivativeConditionalOrdersRequest{}
	subscription, err := broker.Native().Trading().Realtime().SubscribeDerivativeConditionalOrders(ctx, request)
	if err != nil {
		panic(err)
	}
