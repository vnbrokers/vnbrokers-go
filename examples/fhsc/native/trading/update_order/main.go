...
func main() {
	...
	request := nativedto.UpdateOrderOperationRequest{
		SubAccountID: env.RequiredString("FHSC_SUB_ACCOUNT_ID"),
		OrderID:      env.RequiredString("FHSC_ORDER_ID"),
		Body: nativedto.UpdateOrderRequest{
			Quantity: env.RequiredInt("FHSC_QUANTITY"),
			Price:    env.RequiredInt("FHSC_LIMIT_PRICE"),
		},
	}
	response, err := broker.Native().Trading().UpdateOrder(context.Background(), request)
	if err != nil {
		panic(err)
	}
	fmt.Printf("%+v\n", response)
