...
func main() {
	...
	request := nativedto.SubscribePutThroughAdvertisementsRequest{Symbols: env.List("TCBS_SYMBOLS", "1,2,3,4,5")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribePutThroughAdvertisements(ctx, request)
	if err != nil {
		panic(err)
	}
