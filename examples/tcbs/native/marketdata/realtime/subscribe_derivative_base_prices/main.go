...
func main() {
	...
	request := nativedto.SubscribeDerivativeBasePricesRequest{Symbols: env.List("TCBS_SYMBOLS", "VN30F1M")}
	subscription, err := broker.Native().MarketData().Realtime().SubscribeDerivativeBasePrices(ctx, request)
	if err != nil {
		panic(err)
	}
